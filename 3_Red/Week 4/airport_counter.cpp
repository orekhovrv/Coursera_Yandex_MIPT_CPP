...
public:
    ...
    size_t Get(TAirport airport) const {
        return airports_count[CastAirportToNumber(airport)];
    }

    // добавить данный элемент
    void Insert(TAirport airport) {
        airports_count[CastAirportToNumber(airport)]++;
    }
    ...
    void EraseAll(TAirport airport) {
        airports_count[CastAirportToNumber(airport)] = 0;
    }


    static const size_t count = static_cast<uint32_t>(TAirport::Last_);
    ...
    Items GetItems() const {
        Items items;
        for (int i = 0; i < airports_count.size(); i++) {
            items[i] = Item{TAirport(i), airports_count[i]};
        }
        return items;
    }
...
private:
    ...
    static uint32_t CastAirportToNumber(TAirport airport) {
        return static_cast<uint32_t>(airport);
    }

    array<size_t, count> airports_count;
